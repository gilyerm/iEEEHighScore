...
public class Demo_239 {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < a.length; i++) {
            a[i]=arr[0];
            for (int j = 0; j < 5; j++) {
                a[i]=a[i].replaceAll(""+(j+1),j==i?"1":"0");
            }
        }

        List<DecaByte> collect = Arrays.stream(arr).parallel()
                .map(s -> s.replaceAll("2", "1").replaceAll("3", "1")
                        .replaceAll("4", "1").replaceAll("5", "1"))
                .map(DecaByte::new).collect(Collectors.toList());

        List<DecaByte> bits = Stream.of(a).map(DecaByte::new).collect(Collectors.toList());
        ...
        System.out.println("missing:\t"+IntStream.range(0, 1024).filter(value -> Integer.bitCount(value) == 5).boxed().map(integer -> DecaByte.toBin(integer,10)).map(DecaByte::new).filter(new Predicate<DecaByte>() {
            @Override
            public boolean test(DecaByte s) {
                return !collect.contains(s);
            }
        }).map(DecaByte::toStringAsBin).collect(Collectors.toList()));
